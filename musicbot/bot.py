...
class MusicBot(discord.Client):
        ...
        if server.id not in self.players:
            if not create:
                raise exceptions.CommandError(
                    'The bot is not in your voice channel.  '
                    'Use %ssummon to call it over.' % self.config.command_prefix)
    ...
    async def cmd_play(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if 'entries' in info:
            ...
            reply_text = "Added song: **%s** to the queue. Position: %s"
            btext = entry.title
